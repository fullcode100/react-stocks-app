// src/components/ForexPrices.tsx
...
const ForexPrices: React.FC = () => {
  const [prices, setPrices] = useState<ForexData[]>([]);

  useEffect(() => {
    setTimeout(() => webSocketService.subscribeToChannel("XAS.ETH-USD"), 2000);

    webSocketService.setCallback((data) => {
      const newPrices: ForexData[] = data.filter((price : ForexData) => price.ev === 'XAS' && price.pair && price.pair === 'ETH-USD');
      if (newPrices.length) {
    ...
    return () => {
      setTimeout(() => webSocketService.unsubscribeFromChannel("XAS.ETH-USD"), 2000);
    };
  }, []);

  return (
    <div className="p-4">
      <h2 className="text-2xl font-bold mb-4">Forex Prices</h2>
